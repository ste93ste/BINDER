# Test 2D DCT sampling by looking at "closeness" of mean sample for spatial, DCT scipy, and ours.
import os
import numpy as np
...
def test():
    ...
    mask = ((X/.5)**2 + (Y/.7)**2) < 1
    T = mask + np.random.standard_normal((Nx, Ny)) * 100
    ...
    for order in [1, 2]:
        ...
        mean_sample = np.mean(samples, axis=0).reshape((Nx, Ny), order='F')
        ...
        mean_sample_DCT = np.mean(samples, axis=0).reshape((Nx, Ny), order='F')

        if visualizer:
            plt.subplot(1, 5, 3)
            plt.imshow(mean_sample_DCT)
            plt.title("Mean sample DCT")
        ...
        mean_sample_ours = np.mean(our_samples, axis=0).reshape((Nx, Ny), order='F')
        ...
        T_tmp[:, :, 0, 0] = T
        T_tmp = np.ascontiguousarray(T_tmp)
        emptyPositions = np.zeros([Nx, Ny, 1, 2])
        emptyPositions = np.ascontiguousarray(emptyPositions)
        Delta = np.ascontiguousarray(Delta)
        emptyBinnedNodes = np.zeros([Nx, Ny, 1])
        ...
        registratorObj = BINDER.Registration.Registration(final_locations=emptyPositions.ravel(order="C"),
                                                   ...
                                                   Nx_v=Nx, Ny_v=Ny, Nz_v=1, Nx_n=Nx, Ny_n=Ny, Nz_n=1,
                                                   spline_order=3,
                                                   non_zero_voxels=Nx * Ny)
        ...
        mean_sample_ours_pipeline = meanDefVoxelPos[:, :, 0, 0]

        if visualizer:
            plt.subplot(1, 5, 4)
            plt.imshow(mean_sample_ours)
            plt.title("Mean sample ours")
            plt.subplot(1, 5, 5)
            plt.imshow(mean_sample_ours_pipeline)
            plt.title("Mean sample pipeline")
        ...
        assert (np.allclose(mean_sample, mean_sample_DCT, rtol=1e-3, atol=1e-2))
